...
// Aggiungi questo nuovo componente
const StoreAlert = () => {
    const [alerts] = React.useState([
        {
            id: 1,
            message: "Il giorno 27/11 il negozio resterà chiuso per l'intera mattinata",
            type: "info", // può essere: info, warning, danger, success
            active: true, // se false, l'avviso non verrà mostrato
            startDate: "2024-03-27", // data di inizio validità
            endDate: "2024-03-27"    // data di fine validità
        }
        // Puoi aggiungere altri avvisi qui
    ]);

    // Verifica se l'avviso è valido per la data corrente
    const isAlertValid = (alert) => {
        const today = new Date();
        const start = new Date(alert.startDate);
        const end = new Date(alert.endDate);
        return alert.active && today >= start && today <= end;
    };
    ...
    return (
        <div className="store-alerts">
            {alerts.map(alert => (
                isAlertValid(alert) && (
                    <div key={alert.id} className={`alert alert-${alert.type} alert-dismissible fade show m-0`} role="alert">
                        <div className="container">
                            <i className="bi bi-info-circle me-2"></i>
                            {alert.message}
...
const App = () => {
    ...
    return (
        <div className="app-container">
            {/* Aggiungi StoreAlert qui, prima di tutto */}
            <StoreAlert />
